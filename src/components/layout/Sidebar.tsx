...
export function Sidebar({ user, isCollapsed, onToggle, onLogoutRequest }: SidebarProps) {
  ...
  return (
    <aside className={`hidden md:flex flex-col border-r border-slate-200 bg-white h-screen sticky top-0 relative z-40 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'
      }`}>
      {/* Collapse Toggle Button */}
      <button
        onClick={onToggle}
        className="hidden md:flex absolute top-[18px] -right-[14px] z-50 items-center justify-center h-9 w-9 rounded-full border border-slate-200 bg-white text-slate-500 hover:text-slate-800 hover:bg-slate-50 transition-all shadow-md cursor-pointer"
      ...
      {/* Brand Header */}
      <div className={`h-16 flex items-center border-b border-slate-100 gap-2.5 transition-all duration-300 ${isCollapsed ? 'justify-center px-4' : 'px-6'
        }`}>
      ...
      {/* Navigation Links */}
      <nav className={`flex-1 overflow-y-auto py-6 space-y-6 transition-all duration-300 ${isCollapsed ? 'px-2' : 'px-4'
        }`}>
                ...
                .map(item => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center transition-all ${isCollapsed ? 'justify-center p-3 rounded-xl' : 'gap-3 px-3 py-2 rounded-xl'
                        } ${isActive
      ...
      {user && (
        <div className={`border-t border-slate-100 flex flex-col bg-slate-50/50 transition-all duration-300 ${isCollapsed ? 'p-3 items-center gap-4' : 'p-4 gap-3'
          }`}>
          ...
          <button
            ...
            className={`flex items-center justify-center bg-white hover:bg-slate-100 text-slate-600 hover:text-slate-800 text-xs font-bold border border-slate-200 rounded-xl transition-all cursor-pointer ${isCollapsed ? 'p-2.5 w-10 h-10' : 'gap-2 w-full py-2'
              }`}
          >
